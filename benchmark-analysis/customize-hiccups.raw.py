...
def hiccups(M, cM, IR, chromLen, Diags, cDiags, num, chrom, pw=[2], ww=[5], maxww=5,
    ...
    for pi, wi in zip(pw, ww):
        ...
        for fl in flocals:
            ...
            xi, yi = cEM.nonzero()
            Evalues = np.array(cEM[xi, yi]).ravel()
            Mask = Evalues > 0
            ...
            valid_indices = np.where(Evalues > 0)[0]
            if valid_indices.size > 0:
                Poiss = poisson(Evalues[valid_indices])
                pvalue[valid_indices] = 1 - Poiss.cdf(Ovalues[fl][valid_indices])
                qvalue[valid_indices] = multipletests(pvalue[valid_indices], alpha = sig, method = 'fdr_bh')[1]
...
chrom = 'chr21'
outfil = 'K562-H3K27ac.{0}.False.hiccups.txt'.format(chrom)
pw = [2,4]
ww = [5,7]
maxww = 7
maxapart = 3000000
...
cHeatMap = clr.matrix(balance='weight', sparse=True).fetch(chrom)
chromLen = H.shape[0]
...
# run HiCCUPS
pixel_table = hiccups(M, cM, IR, chromLen, Diags, cDiags, num, chrom, pw=pw, ww=ww, maxww=maxww,
                      sig=1, sumq=1, maxapart=maxapart, double_fold=0, single_fold=0, res=res,
                      min_marginal_peaks=3, onlyanchor=True)
